...
export default {
  ...
  effects: {
    ...
    *getRooms(_, { call, put, select }) {
      const { queryForm: { area: { value } }, areaList } = yield select(state => state.machine_roomauth);
    ...
    *updateRoomUser(_, { call, put, select }) {
      const { selectedRowKeys, authForm: { user_no: { value } } } = yield select(state => state.machine_roomauth);
